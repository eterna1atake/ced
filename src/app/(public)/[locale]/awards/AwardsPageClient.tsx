        ...
        <>
            ...
            {isLightboxOpen && (
                ...
                >
                    ...
                    {/* Main Content */}
                    <div
                        className="relative w-full h-full max-w-6xl max-h-[90vh] p-4 flex flex-col md:flex-row items-stretch justify-center gap-6"
                        onClick={(e) => e.stopPropagation()}
                    >
                        {/* Image Section */}
                        <div className="relative w-full md:w-2/3 flex flex-col items-center justify-center">
                            {/* Main Display Image */}
                            <div className="relative w-full h-0 flex-grow min-h-[40vh]">
                                <Image
                                    ...
                                    <div className="flex justify-center gap-2 p-1">
                                        {[sortedAwards[lightboxIndex].image, ...(sortedAwards[lightboxIndex].gallery || [])].map((img, idx) => (
                                            <button
                                                ...
                                                className={`relative h-16 w-16 md:h-20 md:w-20 flex-shrink-0 rounded-md overflow-hidden border-2 transition-all ${selectedImageIndex === idx ? "border-[#35622F] opacity-100" : "border-transparent opacity-60 hover:opacity-100"}`}
                                            >
                                                <Image
                                                    src={img}
                                                    alt={`Thumbnail ${idx + 1}`}
                        ...
                        <div className="w-full md:w-1/3 flex flex-col h-full">
                            <div className="flex-grow bg-slate-100/95 backdrop-blur-xl rounded-2xl border border-slate-200 overflow-y-auto p-6 md:p-8 shadow-2xl custom-scrollbar">
                                {/* Category/Date Badge */}
                                ...
                                <div className="space-y-8">
                                    ...
                                    <div className="pt-6 border-t border-slate-200 flex items-center justify-between text-slate-400 text-[10px] md:text-xs">
                                        ...
                                        <span className="font-mono opacity-70">AWARD-{(sortedAwards[lightboxIndex]._id || sortedAwards[lightboxIndex].id || "").toString().split('-').pop()}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    {/* Counter */}
                    <div className="absolute bottom-6 right-8 text-slate-400 text-sm font-medium">
                        Award {lightboxIndex + 1} / {sortedAwards.length}
                    </div>
                </div>
            )}
        </>
