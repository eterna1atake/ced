                        ...
                        <div className="sticky top-24 space-y-4">
                            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                                <div className="p-4 bg-slate-50 border-b border-slate-200">
                                    <h3 className="font-medium text-slate-800 text-sm uppercase tracking-wider">Categories</h3>
                                </div>
                                <div className="p-2 space-y-1">
                                    {CATEGORIES_CONFIG.map((category) => (
                                        <button
                                            ...
                                            className={`w-full flex items-center justify-between px-4 py-3 rounded-xl font-medium transition-all group ${activeCategory === category.id
                    ...
                    <div className="flex-grow space-y-8">
                        ...
                        <div className="space-y-6">
                            {activeSections.map((section) => {
                                ...
                                return (
                                    <div key={section.id} className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden transition-all hover:shadow-md">
                                        <button
                                        ...
                                        >
                                            <div className="flex items-center gap-4">
                                                <div className={`w-8 h-8 shrink-0 rounded-lg border flex items-center justify-center transition-all duration-300 ${expandedSections[section.id]
                                                    ...
                                                    <FontAwesomeIcon
                                                        icon={faChevronDown}
                                                        className={`w-4 h-4 transition-transform duration-300 ${expandedSections[section.id] ? "rotate-0" : "-rotate-90"}`}
                                                    />
                                                </div>
                                                <span className="font-medium text-slate-800 text-base md:text-lg transition-colors group-hover:text-[#35622F]">
                                                    {isThai ? section.th : section.en}
                                                </span>
                                            </div>
                                            {sectionForms.length > 0 && (
                                                <span className="bg-slate-100 text-slate-500 text-[10px] font-normal px-2 py-0.5 rounded-full uppercase tracking-tighter">
                                                    ...
                                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4">
                                                        {sectionForms.map((item) => (
                                                            <a
                                                                ...
                                                                className="flex items-center gap-4 p-3 rounded-xl border border-slate-100 bg-slate-50/50 hover:bg-white hover:border-[#35622F]/30 hover:shadow-md hover:translate-y-[-2px] transition-all group"
                                                            >
                                                                <div className="w-10 h-10 shrink-0 rounded-lg bg-white border border-slate-100 flex items-center justify-center shadow-sm group-hover:bg-[#35622F] group-hover:text-white transition-all">
