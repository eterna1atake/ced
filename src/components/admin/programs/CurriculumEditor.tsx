...
const CurriculumItemEditor = ({
    ...
    const idError = errors[`${prefix}.id`];
    const titleThError = errors[`${prefix}.title.th`];
    const titleEnError = errors[`${prefix}.title.en`];
            ...
            <div className="flex gap-2 items-start">
                <div className="grid grid-cols-2 md:grid-cols-12 gap-3 flex-1">
                    ...
                    <div className="col-span-2 md:col-span-1">
                        <label className={`block text-[10px] font-bold mb-1 ${idError ? "text-red-500" : "text-slate-400"}`}>{t("programDetails.id")}</label>
                        <input
                            type="text"
                            placeholder="ID"
                            value={item.id}
                            onFocus={() => onClearError?.(`${prefix}.id`)}
                            onChange={(e) => updateField('id', e.target.value)}
                            className={`w-full px-2 py-2 text-sm border rounded-md outline-none focus:ring-2 transition-colors ${idError
                                ? "border-red-500 focus:ring-red-500/20"
                                : "border-slate-200 dark:border-slate-700 focus:ring-primary-main/20 focus:border-primary-main"
                                } bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100`}
                        />
                        {idError && <p className="text-red-500 text-[10px] mt-1">{idError}</p>}
                    </div>
                    {/* Title TH */}
                    <div className="col-span-2 md:col-span-4">
                        <div className="flex justify-between items-center mb-1">
                            <label className={`block text-[10px] font-bold ${titleThError ? "text-red-500" : "text-slate-400"}`}>{t("programDetails.titleTh")}</label>
                            <button
                                type="button"
                        ...
                        <input
                            ...
                            value={item.title.th}
                            onFocus={() => onClearError?.(`${prefix}.title.th`)}
                            onChange={(e) => updateTitle('th', e.target.value)}
                            className={`w-full px-3 py-2 text-sm border rounded-md outline-none focus:ring-2 transition-colors ${titleThError
                                ? "border-red-500 focus:ring-red-500/20"
                                : "border-slate-200 dark:border-slate-700 focus:ring-primary-main/20 focus:border-primary-main"
                                } bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 ${item.isBold ? 'font-bold' : ''}`}
                        />
                        {titleThError && <p className="text-red-500 text-[10px] mt-1">{titleThError}</p>}
                    </div>
                    {/* Title EN */}
                    <div className="col-span-2 md:col-span-4">
                        <label className={`block text-[10px] font-bold mb-1 ${titleEnError ? "text-red-500" : "text-slate-400"}`}>{t("programDetails.titleEn")}</label>
                        <input
                            type="text"
                            placeholder={t("programDetails.titleEn")}
                            value={item.title.en}
                            onFocus={() => onClearError?.(`${prefix}.title.en`)}
                            onChange={(e) => updateTitle('en', e.target.value)}
                            className={`w-full px-3 py-2 text-sm border rounded-md outline-none focus:ring-2 transition-colors ${titleEnError
                                ? "border-red-500 focus:ring-red-500/20"
                                : "border-slate-200 dark:border-slate-700 focus:ring-primary-main/20 focus:border-primary-main"
                                } bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 ${item.isBold ? 'font-bold' : ''}`}
                        />
                        {titleEnError && <p className="text-red-500 text-[10px] mt-1">{titleEnError}</p>}
                    </div>
                    {/* Credits */}
...
export default function CurriculumEditor({ value, onChange, t, errors = {}, onClearError }: CurriculumEditorProps) {
    ...
    return (
        <div className="space-y-6">
            {value.map((sec, secIdx) => {
                const secTitleThError = errors[`sections[${secIdx}].title.th`];
                const secTitleEnError = errors[`sections[${secIdx}].title.en`];
                    ...
                    <div key={secIdx} className="border-b border-gray-100 pb-8 mb-8">
                        ...
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 pb-4 border-b gap-4">
                            <div className="w-full flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                <div>
                                    <div className="flex justify-between items-center mb-1">
                                        <label className={`block text-xs font-semibold ${secTitleThError ? "text-red-500" : "text-slate-500"}`}>{t("programDetails.sectionTitleTh")}</label>
                                        <button
                                            type="button"
                                    ...
                                    <input
                                        type="text"
                                        value={sec.title.th}
                                        onFocus={() => onClearError?.(`sections[${secIdx}].title.th`)}
                                        onChange={(e) => {
                                            updateSection(secIdx, { ...sec, title: { ...sec.title, th: e.target.value } });
                                            onClearError?.(`sections[${secIdx}].title.th`);
                                        }}
                                        className={`w-full px-3 py-2 border rounded-md text-sm font-semibold bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 outline-none focus:ring-2 transition-colors ${secTitleThError
                                            ? "border-red-500 focus:ring-red-500/20"
                                            : "border-slate-200 dark:border-slate-700 focus:border-primary-main"
                                            }`}
                                    />
                                    {secTitleThError && <p className="text-red-500 text-[10px] mt-1">{secTitleThError}</p>}
                                </div>
                                <div>
                                    <label className={`block text-xs font-semibold mb-1 ${secTitleEnError ? "text-red-500" : "text-slate-500"}`}>{t("programDetails.sectionTitleEn")}</label>
                                    <input
                                        type="text"
                                        value={sec.title.en}
                                        onFocus={() => onClearError?.(`sections[${secIdx}].title.en`)}
                                        onChange={(e) => {
                                            updateSection(secIdx, { ...sec, title: { ...sec.title, en: e.target.value } });
                                            onClearError?.(`sections[${secIdx}].title.en`);
                                        }}
                                        className={`w-full px-3 py-2 border rounded-md text-sm font-semibold bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 outline-none focus:ring-2 transition-colors ${secTitleEnError
                                            ? "border-red-500 focus:ring-red-500/20"
                                            : "border-slate-200 dark:border-slate-700 focus:border-primary-main"
                                            }`}
                                    />
                                    {secTitleEnError && <p className="text-red-500 text-[10px] mt-1">{secTitleEnError}</p>}
                                </div>
                        ...
                        <div className="pl-2 mt-4">
                            ...
                            {sec.items.map((item, itemIdx) => (
                                <CurriculumItemEditor
                                    ...
                                    t={t}
                                    errors={errors}
                                    prefix={`sections[${secIdx}].items[${itemIdx}]`}
                                    onClearError={onClearError}
                                    onChange={(newItem) => {
